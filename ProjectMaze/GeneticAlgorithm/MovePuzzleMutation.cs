...
{
    public class MovePuzzleMutation : IMutation
    {
        private double mutationRate;

        private ILogger logger;

        private static string name_ID = "Move Puzzle Mutation";

        public MovePuzzleMutation(double mutationRate)
        {
        ...
        public void mutate(IGenotype geno)
        {
            try
            {
                ...
                int pos = getRandomPuzzlePos(gen);
                ...
                if (pos != -1)
                {
                    ...
                    if (logger != null)
                    {
                        logger.writeLog("Puzzle Move Mutation -- Moved Light from Room " + chosen_spawn[1] + " to Room " + new_room);
                    }
        ...
        private int getRandomPuzzlePos(Genotype gen)
        {
            List<int> pos = new List<int>();

            for (int i = 0; i < gen.getSpawnPoints().Count; i++)
            {
                if (gen.getSpawnPoints()[i][0] == 5)
                {
                    pos.Add(i);
                }
            }
